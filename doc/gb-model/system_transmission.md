...
# Transmission Components {#system-transmission}
...
Before clustering, offshore wind farm stub buses in the OSM network are remapped to their electrically connected onshore regional bus — see [Data cleaning](data_cleaning.md) for details.
...
## Data Sources {#transmission-data-sources}

The figure below gives a high-level view of the transmission pipeline:

```mermaid
flowchart LR
    osm["OSM pre-built<br/>network"]:::blue
    tyndp["ENTSO-E TYNDP<br/>(PyPSA-Eur)"]:::blue
    neso_pdf["NESO transmission<br/>availability reports<br/>(PDF)"]:::blue
    fes_plan["FES interconnector<br/>commissioning plan<br/>(config)"]:::blue
    regions["Merged region<br/>shapes"]:::yellow

    busmap["Offshore busmap<br/>(stub → region)"]:::yellow
    intercon["interconnectors_p_nom.csv<br/>(MW per year)"]:::yellow
    avail_intra["intra_gb transmission<br/>availability (monthly)"]:::yellow
    avail_inter["inter_gb transmission<br/>availability (monthly)"]:::yellow

    network(["PyPSA Network<br/>(Line, Link)"]):::green

    osm --> busmap
    regions --> busmap
    osm -->|"AC lines (clustered)"| network
    tyndp -->|"intra-EU DC links"| network
    busmap -->|"offshore bus mapping"| network
    fes_plan --> intercon
    regions --> intercon
    intercon -->|"GB DC links (p_nom per year)"| network
    neso_pdf --> avail_intra
    neso_pdf --> avail_inter
    avail_intra -->|"line s_nom scaling"| network
    avail_inter -->|"link p_nom scaling"| network
...
The internal GB transmission grid topology is read from a [custom version of the OpenStreetMap (OSM) pre-built network](https://doi.org/10.5281/zenodo.18712831) maintained by the PyPSA-Eur pipeline.
...
See [our interconnector GitHub issue](https://github.com/open-energy-transition/gb-dispatch-model/issues/232) for more details.
...
## System Components {#transmission-components}

<a id="transmission-ac-lines"></a>
<a id="transmission-interconnectors"></a>
...
*PyPSA network component attributes — AC Lines and DC Interconnectors*

| Component | Attribute | Static / Dynamic | Source | Notes |
|---|---|---|---|---|
| `Line` | `bus0`, `bus1` | Static | OSM + clustering | Regional AC buses at each end; assigned during the PyPSA-Eur clustering step |
| `Line` | `s_nom` | Static | OSM | Thermal rating (MVA) before availability scaling; derived from OSM voltage and line type |
| `Line` | `x`, `r` | Static | OSM | Series reactance and resistance (p.u.) from OSM line type and length; used in power-flow calculations |
| `Line` | `num_parallel` | Static | OSM | Number of parallel circuits; aggregated during clustering |
| `Line` | `type` | Static | OSM | Standard line type string (e.g. `"Al/St 240/40 2-bundle 380.0"`); determines per-unit-length impedance |
| `Line` | `s_nom_extendable` | Static | Default (`False`) | Capacity is fixed; the model does not invest in new AC lines |
| `Line` | `s_nom_pu` | Dynamic (hourly) | Workflow (NESO PDFs) — *not yet applied* | Hourly availability series derived by random-sampling the monthly TO-zone unavailability fraction with a fixed seed per zone. Currently processed but not applied to line ratings; the intent is to apply TO-zone availability to boundary transfer capabilities rather than individual line `s_nom` values (see [Transmission Availability](#transmission-availability)) |
| `Link` (DC) | `bus0`, `bus1` | Static | Workflow (config + regions) | `bus0` is the GB regional AC bus nearest to the configured lat/lon; `bus1` is the AC bus of the neighbouring country |
| `Link` (DC) | `carrier` | Static | Workflow | Set to `"DC"` for all GB cross-border interconnectors |
| `Link` (DC) | `p_nom` | Static (per model year) | Workflow (FES config plan) | One-directional nameplate transfer capacity (MW); scenario- and year-dependent. Projects are accumulated forward in time so a commissioned link retains its capacity in all later years |
| `Link` (DC) | `p_min_pu` | Static | Workflow (`-1`) | Set to `-1` to enable full bidirectional flow; combined with `p_nom` this allows power up to the nameplate rating in either direction |
| `Link` (DC) | `p_nom_extendable` | Static | Default (`False`) | Capacity is fixed to the FES plan value; no investment in additional interconnector capacity |
| `Link` (DC) | `efficiency` | Static | Default (`1`) | No transmission losses modelled |
| `Link` (DC) | `p_nom_pu` | Dynamic (monthly) | Workflow (NESO PDFs) | Monthly availability fraction; a single value shared across all interconnectors per month, averaged over all projects and report years (see [Transmission Availability](#transmission-availability)) |

## Configuration {#transmission-configuration}

Example interconnector option entry (one entry per project under `interconnectors.options`):

```yaml
{{ yaml_snippet("config.gb.2024.yaml", "doc:interconnector-option-example-start", "doc:interconnector-option-example-end") }}
```

Interconnector commissioning plan (maps FES scenarios to project lists by year):

```yaml
{{ yaml_snippet("config.gb.2024.yaml", "doc:interconnector-plan-start", "doc:interconnector-plan-end") }}
```

Transmission availability data configuration:

```yaml
{{ yaml_snippet("config.gb.2024.yaml", "doc:transmission-availability-config-start", "doc:transmission-availability-config-end") }}
```

## Implementation Notes {#transmission-implementation-notes}

### Data Processing Workflow

The transmission system is built through a pipeline implemented in `rules/gb-model/transmission.smk`:

![](img/transmission_workflow.svg)
...
### Key Assumptions {#system-transmission-assumptions}
...
- **Interconnector capacity plan**: The mapping of FES scenarios to specific projects and commissioning years is a best-guess derived by comparing cumulative interconnector capacities against the FES headline figures — see [our interconnector GitHub issue](https://github.com/open-energy-transition/gb-dispatch-model/issues/232) for details
- **No power-flow physics**: Kirchhoff Voltage Law constraints are removed from the whole network in *both* the dispatch and redispatch optimisations (`remove_KVL_constraints`). Flows are therefore a transport problem: line reactances do not influence how power splits between parallel paths, and loop flows are not represented. The `x` and `r` attributes listed above are carried through the workflow but have no effect on the solved flows
- **No transmission losses**: `solving.options.transmission_losses` is `0` and every DC link has `efficiency = 1`, so no energy is lost in transmission anywhere in the model. As a consequence, the interconnector loss term $\eta_\text{loss}$ in the [bid/offer equations](system_dispatch_redispatch.md#system-redispatch-assumptions) evaluates to zero
- **Boundary capabilities are not FES-consistent**: future ETYS boundary capabilities are read from `etys.future_capacities_sheet_name` (default: the ETYS 2022 chart data, which follows the NOA 2022 recommendations). FES 2024 scenario codes do not appear in that dataset, so the *average* capability across all ETYS scenarios is used for every FES scenario. Boundary B3b is instead overridden with hardcoded values from `etys.manual_future_capacities`, taken from the ETYS 2022 report's "Leading the Way" scenario
- **Capability extrapolation**: boundary capabilities are forward-filled beyond the last year present in the ETYS data, i.e. no further reinforcement is assumed after the end of the ETYS horizon. Boundaries present in the capability data but with no matching lines or links in the clustered network are skipped (logged as a warning), so their capability is effectively unlimited
- **Symmetric interconnector capacity**: a single `capacity_mw` per project bounds both import and export. Where the interconnector register gives different import and export maxima, the value rounded to one significant figure is used
- **Interconnector set is replaced wholesale**: PyPSA-Eur's own GB↔neighbour DC links are removed from the network and replaced entirely by the projects in `interconnectors.plan`; nothing from the base network survives

### Offshore Bus Mapping {#transmission-offshore-busmap}
...
!!! info "See also"
    [Data cleaning](data_cleaning.md) — detailed description of the offshore stub problem, the mapping algorithm, and illustrative figures.

### Transmission Availability {#transmission-availability}
...
!!! info "See also"
    ...
    **External Resources**:
    ...
    - [OpenStreetMap / PyPSA-Eur OSM network](https://github.com/PyPSA/pypsa-eur) - Base grid topology
    - [OSM pre-built network dataset (Zenodo)](https://doi.org/10.5281/zenodo.18712831) - gb-dispatch-model custom OSM archive (version 2026-02-20)
    - [NESO transmission availability reports](https://www.neso.energy/) - Source of intra-GB and interconnector availability data
    - [ENTSO-E TYNDP](https://tyndp.entsoe.eu/) - Cross-border interconnector project reference
