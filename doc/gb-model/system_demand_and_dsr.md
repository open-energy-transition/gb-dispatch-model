...
# Baseline demand & demand-side response (DSR) {#system-demand_and_dsr}
...
### Model Representation {#model_representation}
...
flowchart LR
    AC_bus((AC Bus)):::acbus
    Sector_bus((Sector Bus)):::sector
    Sector_DSR_bus((Sector DSR Bus)):::sector
    heat_load[Sector Load]:::peach
    sector_dsr_store[Sector DSR Store]:::lemon

    AC_bus -->|"unmanaged load link"| Sector_bus
    Sector_bus --> heat_load
    Sector_bus -->|charge| Sector_DSR_bus
    Sector_DSR_bus -->|discharge| Sector_bus
    Sector_DSR_bus <--> sector_dsr_store
...
Whilst the general structure is consistent across demand types, some extra components are added for specific sectors which can be found in the corresponding documentation sections (see [Electric Vehicle Subsystem](system_ev.md), [Heat System](system_heat.md), [Hydrogen Subsystem](system_hydrogen.md)).
...
<a id="demand_and_dsr_data_sources"></a>
...
Specific data requirements are explained in detail in the corresponding sections of the documentation (e.g., [Heat System](system_heat.md#system-heat) uses ES2 from the FES workbook for European demand assumptions).
...
```yaml
{{ yaml_snippet("config.gb.2024.yaml", "doc:demand-start", "doc:demand-end") }}
```

DSR is configured in the model configuration files under the `fes.gb.flexibility` section:

```yaml
{{ yaml_snippet("config.gb.2024.yaml", "doc:flexibility-start", "doc:flexibility-end") }}
```
...
The representation of each demand sector is illustrated in the [Model Representation](#model_representation) section, which provides detailed descriptions of the components and their interactions.
...
![](img/demand_and_dsr_workflow.svg)
...
### Key Assumptions {#system-demand_dsr-assumptions}
...
!!! info "See also"
    **Related Documentation**:
    ...
    - [Heat System](system_heat.md) - Heat demand details
    - [Hydrogen Subsystem](system_hydrogen.md) - Hydrogen demand details
    - [Electric Vehicle Subsystem](system_ev.md) - EV demand details
